...
// floor + system Member row. Runtime DB assertions live in
// web/evals/phase-0/floor.spec.ts (Playwright against running Supabase).
...
describe('T042 — migrations directory state after T042', () => {
  it('contains the T041 + T042 set after this ticket', () => {
    const files = readdirSync(MIGRATIONS_DIR).filter((f) => f.endsWith('.sql')).sort()
    expect(files).toEqual([
      '001_extensions.sql',
      '002_members_floor.sql',
      '002a_member_events.sql',
      '002b_system_member.sql',
...
describe('T042 — 002_members_floor.sql', () => {
  const raw = read('002_members_floor.sql')
  ...
  it('does NOT declare a FK from members.id to auth.users (deferred to Phase 1)', () => {
    // No `references auth.users` clause on the id column.
  ...
  it('reserves home_location_id and primary_group_id WITHOUT a FK (Phase 1 lands those tables)', () => {
  ...
  it('does NOT create an INSERT or DELETE policy (action-layer-only writes)', () => {
    expect(sql).not.toMatch(/create policy[^;]+for insert/i)
    expect(sql).not.toMatch(/create policy[^;]+for delete/i)
  })
})

describe('T042 — 002a_member_events.sql', () => {
  const raw = read('002a_member_events.sql')
  const sql = stripComments(raw)
  ...
  it('enforces the b1 event_kind enum via CHECK constraint', () => {
    ...
    // Spot-check a few values from the b1 enum
  ...
  it('enables RLS with read-only policy (no INSERT / UPDATE / DELETE)', () => {
  ...
  it('defines the partition-rotation function and seeds three months', () => {
...
describe('T042 — 002b_system_member.sql', () => {
  const raw = read('002b_system_member.sql')
  ...
  it('sets login_disabled = true for the system Member', () => {
    expect(raw).toMatch(/insert into\s+public\.members[\s\S]+true/i)
  })
  ...
  it('inserts the self-bootstrap member.created event', () => {
    ...
    // Self-bootstrap: acting_member_id = member_id = SYSTEM_MEMBER_ID.
    ...
    expect(occurrences).toBeGreaterThanOrEqual(3) // members.id, member_events.member_id, member_events.acting_member_id
  })
})
